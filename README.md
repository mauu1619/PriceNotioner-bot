...
- **Парсинг:** `httpx` (асинхронные HTTP запросы) + `fake-useragent` (подмена заголовков) + SOCKS5 прокси.
...
- `PROXY_URL`: SOCKS5 прокси для парсинга (по умолчанию прописан в примере, но желательно использовать свой, чтобы не поймать блокировку маркетплейса).
...
- **Меню: Мои подписки** - Просмотр текущего тарифа и лимитов. Возможность повысить тариф до PRO за Telegram Stars.
